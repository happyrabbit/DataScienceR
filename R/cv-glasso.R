...
#' @param type  the type of prediction. type = 'link' is on the scale of linear predictors (default), whereas type = 'response' is on the scale of the response  variable, i.e. type = 'response' applies the inverse link function to the linear predictors.  
...
cv.glasso <- function(trainx, trainy, nlam = 100, type = "link", kfold = 10, 
  ...
  res <- list(lambda=lambda, pred = pre, auc = auc, log_likelihood = loglike, maxrho = maxco)
  return(res)
}
